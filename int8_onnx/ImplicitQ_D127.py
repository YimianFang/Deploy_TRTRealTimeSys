...
class Decoder(nn.Module):
    def __init__(self, out_channel_N=128, out_channel_M=192):
        ...
        self.deconv2.bias.data = fltm["Decoder.deconv2.bias"].round()
        ...
        self.deconv3.bias.data = fltm["Decoder.deconv3.bias"].round()
        ...
        self.deconv4.bias.data = fltm["Decoder.deconv4.bias"].round()

        self.mulsD0 = torch.tensor(np.load("int8onnxdata/mulsD0.npy")).cuda()
        self.mulsD1 = torch.tensor(np.load("int8onnxdata/mulsD1.npy")).cuda()
        self.mulsD2 = torch.tensor(np.load("int8onnxdata/mulsD2.npy")).cuda()
        self.mulsD3 = torch.tensor(np.load("int8onnxdata/mulsD3.npy")).cuda()
    ...
    def forward(self, x):
        ...
        x = self.deconv2(x - 127) ### .to(torch.int32)
        x = x + self.D2_127plus
        x = x * self.mulsD1
        x = torch.floor((x + 4096) >> 13)
        ...
        x = self.deconv3(x - 127) ### .to(torch.int32)
        x = x + self.D3_127plus
        x = x * self.mulsD2
        x = torch.floor((x + 4096) >> 13)
        ...
        x = self.deconv4(x - 127) ### .to(torch.int32)
        x = x + self.D4_127plus
        x = x * self.mulsD3
        x = torch.floor((x + 2 ** 21) >> 22) #####/255 未除以255

        # torch.save(x, "trtoutput/x86_D_xhat.pth.tar")
...
model = Decoder().cuda()
input = torch.load("trtoutput/x86_E_y.pth.tar").cuda()
model.eval()
out = model(input)
...
torch.onnx.export(model, dummy_input, "int8LIC_D.onnx", verbose=True, export_params=True, input_names=input_names, output_names=output_names, opset_version=9) #####, dynamic_axes=dynamic_axes)
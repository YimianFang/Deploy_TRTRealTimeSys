...
class priorDecoder(nn.Module):
    def __init__(self, out_channel_N=128, out_channel_M=192):
        ...
        self.deconv2.bias.data = fltm["priorDecoder.deconv2.bias"].round()
        ...
        self.deconv3.bias.data = fltm["priorDecoder.deconv3.bias"].round()

        self.mulspD0 = torch.tensor(np.load("int8onnxdata/mulspD0.npy")).cuda()
        self.mulspD1 = torch.tensor(np.load("int8onnxdata/mulspD1.npy")).cuda()
        self.mulspD2 = torch.tensor(np.load("int8onnxdata/mulspD2.npy")).cuda()
    ...
    def forward(self, x):
        ...
        # torch.save(x, "trtoutput/x86_pD_sigma.pth.tar")
...
torch.onnx.export(model, dummy_input, "int8LIC_pD.onnx", verbose=True, export_params=True, input_names=input_names, output_names=output_names, opset_version=9) #####, dynamic_axes=dynamic_axes)
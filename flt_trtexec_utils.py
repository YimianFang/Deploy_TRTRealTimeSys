...
# e = torch.load("flt_entropy_bottleneck.pth.tar", map_location=torch.device('cpu'))
# entropy_bottleneck._offset = nn.Parameter(e["_offset"], requires_grad=False)
# entropy_bottleneck._quantized_cdf = nn.Parameter(e["_quantized_cdf"], requires_grad=False)
# entropy_bottleneck._cdf_length = nn.Parameter(e["_cdf_length"], requires_grad=False)

# g = torch.load("flt_gaussian_conditional.pth.tar", map_location=torch.device('cpu'))
# gaussian_conditional._offset = nn.Parameter(g["_offset"], requires_grad=False)
# gaussian_conditional._quantized_cdf = nn.Parameter(g["_quantized_cdf"], requires_grad=False)
# gaussian_conditional._cdf_length = nn.Parameter(g["_cdf_length"], requires_grad=False)
# gaussian_conditional.scale_table = nn.Parameter(g["scale_table"], requires_grad=False)

# entropy_bottleneck.load_state_dict(e)
# gaussian_conditional.load_state_dict(g)

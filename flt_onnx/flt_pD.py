...
pre = torch.load("trt_flt/pD_dict.pth.tar")
model.load_state_dict(pre)
input = torch.ones([1, 128, 8, 8]).cuda()
model.eval()
out = model(input)
...
torch.onnx.export(model, dummy_input, "trt_flt/flt_pD.onnx", verbose=True, export_params=True, input_names=input_names, output_names=output_names, opset_version=9) #####, dynamic_axes=dynamic_axes)
...
E_engine_file_path = "int8LIC_E_127_XA.trt"
E_f = open(E_engine_file_path, "rb")
E_engine = runtime.deserialize_cuda_engine(E_f.read())
pD_engine_file_path = "int8LIC_pD_127_XA.trt"
...
while True:
    if not frames.empty():
        ...
        image = frame.transpose((2,0,1)) / 255
        input_data = np.ascontiguousarray(np.round(image[np.newaxis, :] * (2 ** 8)), dtype=np.float32)
        #y, z = EpE_exec(E_engine, input_data)
        #yint = y.astype('int8')
        ...
        jmsg = cv2.imencode("pic.jp2", frame, [int(cv2.IMWRITE_JPEG2000_COMPRESSION_X1000), 60])[1]
        ...
        msglen = len(msg)
        msg_len_bs = struct.pack("2i", msglen, jmlen)
        s.send(msg_len_bs)
        print(msglen - jmlen)
        s.send(msg)

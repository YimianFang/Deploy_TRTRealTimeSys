...
def producerun(queue, queueJ, queuelen):
    ...
    while conn:
        frame_idx += 1
        bs_msg_len = conn.recv(8)
        bs_len, blen = struct.unpack("2i", bs_msg_len)
        print(bs_len)
        s_byte = bytes()
...
pD_engine_file_path = "int8LIC_pD_127_TX_flt.trt"
pD_f = open(pD_engine_file_path, "rb")
pD_engine = runtime.deserialize_cuda_engine(pD_f.read()) 
D_engine_file_path = "int8LIC_D_127_TX_flt.trt"
...
def comsumerun(queue, queueJ, queuelen):
    ...
    while True:
      if not queue.empty():
        ...
        x_hat_255,_ = D_exec(D_engine, y_hat) #0.08
        ...
        resize_imgJ = cv2.resize(imgJ, (512,512), interpolation=4)
